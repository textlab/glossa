...
class WaveformPlayerController < ActionController::Base
  ...
  def old_glossa_generate_audio(corpus_id, line_key, start, stop, output_path, file, fname)
    ...
    if !File.exists? "#{fname}.mp3"
      File.symlink("/var/www/html/glossa_media/#{corpus_id}/audio/#{glossa_fn}.mp3", "#{fname}.mp3")
    end
  end

  def glossa_generate_audio(corpus_id, line_key, start, stop, output_path, file, fname)
    if corpus_id.match(/^\d+$/)
      ...
      ActiveRecord::Base.establish_connection(Rails.configuration.database_configuration[ENV["RAILS_ENV"] ||
                                                                                         "development"])
      glossa_fn = old_glossa_filename(basename, corpus_id)
      local_path = "/var/www/html/glossa_media/#{corpus_id}/audio"
      movie_loc = "#{glossa_fn}.mp3"
    else
      raise "Wrong corpus_id: #{corpus_id}"
    end

    start_time = sprintf("%d.%02d.%d", start.to_i / 60, start.to_i % 60, start.gsub(/^[^.]*(\.|$)/, "")[0..1].to_i)
    stop_time = sprintf("%d.%02d.%d", stop.to_i / 60, stop.to_i % 60, stop.gsub(/^[^.]*(\.|$)/, "")[0..1].to_i)
    system("mp3splt", "-q", Rails.root.join(local_path, URI.decode(movie_loc)).to_s, start_time, stop_time,
                       "-d", output_path.to_s, "-o", file)
    if !File.exists? "#{fname}.wav"
      system("ffmpeg", "-nostdin", "-i", "#{fname}.mp3", "-ac", "1", "#{fname}.wav")
    end
  end
  ...
  def show
    ...
    if (!File.exists?("#{fname}-0-wave.jpg"))
      glossa_generate_audio(corpus_id, line_key, start, stop, output_path, @file, fname)
