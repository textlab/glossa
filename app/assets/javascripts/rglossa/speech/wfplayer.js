...
function PagePlayer() {
  ...
  this.getPreviousItem = function(o) {
    ...
    if (o.nodeName.toLowerCase() !== 'li') {
      ...
      return o.getElementsByTagName('span')[0];
    }
  };
  ...
  var stopFunc = function(th) {
    pl.removeClass(th._data.oLI,th._data.className);
    th._data.className = pl.css.sPaused;
    pl.addClass(th._data.oLI,th._data.className);
    self.setPageTitle();
    self.resetPageIcon();
  }
  ...
  this.events = {
    ...
    onload: function() {
      if (!this.loaded) {
        var oTemp = this._data.oLI.getElementsByTagName('span')[0],
    ...
    whileplaying: function() {
      ...
      if (pl.dragActive || !pl.config.useThrottling) {
        ...
        //document.getElementById("debug").innerHTML = document.getElementById("small").offsetLeft + ' ' + document.getElementById("small").offsetWidth;
      } else {
        d = new Date();
      ...
      if (false) {
      small = document.getElementById("small0");
      smallTop = small.offsetTop;
      smallLeft = small.offsetLeft;
      smallHeight = small.offsetHeight;
      smallWidth = 0;
      for(i=0; elem = document.getElementById('small'+i); i++) {
        smallWidth += elem.offsetWidth;
      }
      ...
      div.style.top = small.offsetTop + 'px';
      div.style.left = (smallLeft + smallWidth*this.position/self.getDurationEstimate(this)) + 'px';
      div.style.position = "absolute";
      if (Math.abs(this.position - self.getDurationEstimate(this)) < 2) {
        div.style.visibility = 'hidden';
      }
      }
      //document.getElementById("debug").innerHTML = smallHeight;

      if (this.position >= this.stopPos) {
        this.stop();
        this.setPosition(this.startPos);
      }

      frameX = total_width*this.position/self.getDurationEstimate(this);
      leftVisible = document.getElementById('iframe').contentWindow.document.body.scrollLeft;

      frameWidth = document.getElementById('iframe').offsetWidth;
      rightVisible = leftVisible + frameWidth;
      if (frameX < leftVisible || frameX > rightVisible - 5) {
  ...
  this.init = function(oConfig) {
    ...
    if (self.config.autoStart) {
      ...
      pl.handleClick({target:pl.getByClassName('playlist', 'ul')[0].getElementsByTagName('span')[0]});
    }
