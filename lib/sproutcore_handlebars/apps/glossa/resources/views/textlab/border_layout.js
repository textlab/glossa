...
Textlab.BorderLayoutView = SC.TemplateView.extend({
    ...
    south, or center), and the available options are as follows:
        ...
        - minSize {Integer}: Minimum region size in pixels (height for north and south regions;
              width for east and west).
        - maxSize {Integer}: Maximum region size in pixels (height for north and south regions;
              width for east and west).
    @property {Object}
     */
    regions: {},
    ...
    _makeRegionsResizable: function() {
        if ((this.regions.north && this.regions.north.resizable) ||
            ...
            this.$northRegion.resizable({
                handles: 's',
                minHeight: this.regions.north && this.regions.north.minSize,
                maxHeight: this.regions.north && this.regions.north.maxSize,
                ...
                resize: $.proxy(function() {
                    ...
                    var height = this.$northRegion.outerHeight(true);
        ...
        if ((this.regions.west && this.regions.west.resizable) ||
            ...
            this.$westRegion.resizable({
                handles: 'e',
                minWidth: this.regions.west && this.regions.west.minSize,
                maxWidth: this.regions.west && this.regions.west.maxSize,
                resize: $.proxy(function() {
                // Resizing the west or east regions only affects the center region
                this.$centerRegion.css({ left: this.$westRegion.outerWidth(true) });
            }, this)});
        }

        if ((this.regions.east && this.regions.east.resizable) ||
                this.$eastRegion.hasClass('tl-resizable')) {
            this.$eastRegion.resizable({
                handles: 'w',
                minWidth: this.regions.east && this.regions.east.minSize,
                maxWidth: this.regions.east && this.regions.east.maxSize,
                resize: $.proxy(function() {
                this.$centerRegion.css({ right: this.$eastRegion.outerWidth(true) });
            }, this)});
        }
        ...
        if ((this.regions.south && this.regions.south.resizable) ||
            ...
            this.$southRegion.resizable({
                handles: 'n',
                minHeight: this.regions.south && this.regions.south.minSize,
                maxHeight: this.regions.south && this.regions.south.maxSize,
                start: $.proxy(function() {
                    this.$westRegion.css({ height: '' });
                    this.$eastRegion.css({ height: '' });
                }, this),
                resize: $.proxy(function() {
                    var height = this.$southRegion.outerHeight(true);
        ...
        // Use resizing cursors that point both ways - more intuitive for this purpose
        this.$('.ui-resizable-e, .ui-resizable-w').css({ cursor: 'ew-resize' });
        this.$('.ui-resizable-s, .ui-resizable-n').css({ cursor: 'ns-resize' });
    }
});
