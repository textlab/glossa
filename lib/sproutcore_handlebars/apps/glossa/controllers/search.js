...
/** @scope Glossa.searchController.prototype */ {
  ...
  addQueryRow: function(button) {
    var search = this.get('content');
    var queryRows = search.get('queries');
    queryRows.pushObject(this._createQueryRow());
    search.set('queries', queryRows);
    Glossa.store.dataHashDidChange(search.storeKey);
  },
  ...
  search: function() {
    ...
    var sceneView = Glossa.mainPage.mainPane.get('mainSceneView');
    sceneView.set('nowShowing', sceneView.get('scenes').objectAt(1));
  },
  ...
  _createQueryTermHash: function() {
    return {min: null, max: null, string: ''};
  },


  /** @private */
  _createQueryRow: function() {
    return Glossa.Query.create({corpus: 'dickens', terms: [this._createQueryTermHash()]});
  }
